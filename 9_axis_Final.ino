...
void setupGyro()
{
  ...
  // 2 = 59.5    5 = 476
  // 3 = 119     6 = 952
  imu.settings.gyro.sampleRate = 6; // 59.5Hz ODR
  // [bandwidth] can set the cutoff frequency of the gyro.
  // Allowed values: 0-3. Actual value of cutoff frequency
  // depends on the sample rate. (Datasheet section 7.12)
  imu.settings.gyro.bandwidth = 2;
...
void printSensorReadings()
{
  ...
  float c=imu.calcGyro(imu.gz);
 
  if(c>0){
  Serial.print("The speed in forward direction is=");
  Serial.print(c*0.016956,2);
  Serial.println("km/hr");
  Serial.print("RPM: ");
  Serial.println(c/6,2);
  }
  else{
  Serial.print("The speed in reverse direction is=");
  Serial.print(-c*0.016956,2);
  Serial.println(" km/hr");
  Serial.print("RPM: ");
  Serial.println(-c/6,2);
...
void printAttitude(float ax, float ay, float az, float mx, float my, float mz)
{
  ...
  Serial.print("Camber, Toe: ");
  Serial.print(pitch, 2);
  Serial.print(", ");
  Serial.println(roll, 2);
  Serial.print("Yaw: "); Serial.println(heading, 2);
}
